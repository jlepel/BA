...
(\cite{Masak200911}).
Da das geforderte Framework Sinatra diese Konzept nativ umsetzen kann, lässt sich das Entwurfsmuster entsprechend gut anwenden.


\section{Server}
Um den Server nach den Systemarchitekturen aus Kapitel \ref{sec:Systemarchitektur} zu konzepieren, wird der dieser wie folgt Aufgeteilt.

\begin{figure}[htbp]
  \centering
  \fbox{
\includegraphics[width=0.4\textwidth]{../Bilder/MVC_Eigen.png}
  }
  \caption{Model-View-Controller (\cite{Wikipedia:2015:Online})}
	\label{fig:MVC}
\end{figure}
...
Da der Server eine zentrale Rolle im Entwurf einnimmt, wird im fol
Bevor auf die Anwendung der Entwufsmuster  eingegangen wird, 
...
Ausgehend vom Systemkontext, wird das System hierarchisch zergliedert. Somit können Ebenen in unterschiedlichem Detailgrad entstehen. Ebene 0 stellt das System als Blockbox dar, was der Kontextsicht aus Abbildung \ref{fig:kontextsicht} entspricht.
...
In diesem Fall wird auf dem Client nur die Weboberfläche des VM-Builders aufgerufen um Interaktionen durchführen zu können. Die Logik für den Client ist komplett auf dem Applikationsserver implementiert.
