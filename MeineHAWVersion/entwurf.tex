...
Die benötigte rationale Datenbank kann z.B durch MySQL umgesetzt werden. Sie wird für das Speichern von Konfigurationen einzelner virtueller Maschinen genutzt und für das zu Verfügungstellung der zu installierenden Softwarekomponenten.
...
\end{figure}
\FloatBarrier
Wie in der Verteilungssicht (Abbildung \ref{fig:Verteilungssicht}) zu beschrieben, wird der Server eine relationale Datenbank bereit halten, deren Zuständigkeitsbereich im Speichern von Konfigurationen bereits gebauter virtuellen Maschinen und der Verwaltung von Softwarebestandteilen liegt.
Der in Abbildung \ref{fig:Entity-Relationship-Model} veranschaulichte konzeptionelle Entwurf, zeigt das Tabellenkonstrukt für die angestrebte Datanbank in der Notation des Entity-Relationship-Model.
Der Abschnitt \ref{sssub:RelationalDatabaseDiagram} wird dabei die tabellarische Umsetzung des Konzeptes erklären.\par

Die Tabelle \textbf{Machine} soll zukünftig jede Machine beinhalten, die durch das System aufgebaut wurde. Eindeutig identifizert wird jede Maschine über die \textbf{machine_id}, die fortlaufend generiert wird. Zusätzlich wird der \textbf{Name} der Maschine gespeichert inkl. einer optionalen Beschreibung(Description).
Bei der Erstellung einer virtuellen Maschine kann entschieden werden, ob Software mit installiert wird.
Diese Auswahl wird durch die Tabelle \textbf{Software} bereitgestellt. Wobei jeder Eintrag aus einer automatisch generierten Software_ID besteht, dem Namen der gespeicherten Software, einer optionalen Beschreibung und der zwingend notwendigen Befehlszeile(Command), die den Linux-Befehlt enthält, mit dem die Software installiert wird. 

(Dateien mit installieren?! wie denn?)
...
\subsubsection{RelationalDatabaseDiagram}\label{sssub:RelationalDatabaseDiagram}
\begin{figure}[htb]
  \centering
\includegraphics[width=0.9\textwidth]{../Bilder/RelationalDatabaseDiagram.png}
  \caption{RelationalDatabaseDiagram}
...
In dem vorherigen Kapitel ... Abbildung ... wird die Interaktion zwischen Client-Aufrufen und serverseitigen Reaktionen genauer betrachtet.
